...
Интерфейс lo — это программный интерфейс, предназначенный для тестирования локальной связи.
Любой трафик, отправленный на 127.0.0.1 (localhost), возвращается к тому же устройству без выхода в сеть.
...
Sudo — команда, которая наделяет правами суперпользователя.
...
![alt text](<screenshots/image copy 11.png>)
...
![alt text](<screenshots/image copy 12.png>) :wq vim
![alt text](<screenshots/image copy 13.png>) ctrl+o enter ctrl+x nano
![alt text](<screenshots/image copy 14.png>) F2 F10 mcedit

##### Используя каждый из трех выбранных редакторов, открой файл на редактирование, отредактируй файл, заменив никнейм на строку «21 School 21», закрой файл без сохранения изменений.
- В отчёт вставь скриншоты:
    - Из каждого редактора с содержимым файла после редактирования.
- В отчёте укажи, что сделал для выхода без сохранения изменений.
![alt text](<screenshots/image copy 15.png>) :q! vim
![alt text](<screenshots/image copy 16.png>) ctrl+x
![alt text](<screenshots/image copy 17.png>) F10 не сохранять mcedit
![alt text](<screenshots/image copy 18.png>) cat для всех файлов

##### Используя каждый из трех выбранных редакторов, отредактируй файл ещё раз (по аналогии с предыдущим пунктом), а затем освой функции поиска по содержимому файла (слово) и замены слова на любое другое.
- В отчёт вставь скриншоты:
    - Из каждого редактора с результатами поиска слова.
    - Из каждого редактора с командами, введёнными для замены слова на другое.
![alt text](<screenshots/image copy 19.png>) vim
![alt text](<screenshots/image copy 20.png>) nano
![alt text](<screenshots/image copy 21.png>) mcedit
![alt text](<screenshots/image copy 22.png>) cat для всех файлов
...
![alt text](<screenshots/image copy 23.png>)
##### Перенастрой службу SSHd на порт 2022.
![alt text](<screenshots/image copy 24.png>)
##### Используя команду ps, покажи наличие процесса sshd. Для этого к команде нужно подобрать ключи.
- В отчёте объясни значение команды и каждого ключа в ней.
![alt text](<screenshots/image copy 25.png>) ps показывает запущенные процессы  a: Показать процессы всех пользователей (не только текущего).
u: Отобразить информацию о пользователе, запустившем процесс, и другие подробности (например, используемую память, процессорное время и т. д.).
x: Показать процессы, которые не привязаны к терминалу (например, фоновые службы, такие как sshd). | это конвеер который передает вывод команды ps aux в команду grep которая возвращает строки в которых содержится sshd
...
![alt text](<screenshots/image copy 26.png>)
-t: Отображает только TCP-соединения.
-a: Показывает все соединения, включая ожидающие входящих подключений (LISTEN).
-n: Выводит IP-адреса и номера портов в числовом формате (не преобразует в доменные имена и сервисы, например, 80 вместо http).
Proto: Протокол (здесь всегда tcp, так как мы указали -t).
Recv-Q: Количество байтов в очереди получения, ожидающих обработки приложением (обычно 0).
Send-Q: Количество байтов в очереди отправки, ожидающих подтверждения (обычно 0).
Local Address: Локальный адрес (IP-адрес устройства и порт). Формат IP:PORT.
  Например: 0.0.0.0:22 означает, что порт 22 открыт на всех интерфейсах устройства.
Foreign Address: Удалённый адрес (IP-адрес и порт подключенного устройства). Формат IP:PORT.
  Например: 192.168.1.101:50312 — это IP-адрес и порт клиента, подключённого к локальному устройству.
State: Состояние соединения. Возможные значения:
  LISTEN: Ожидание входящих соединений.
  ESTABLISHED: Установленное соединение.
  CLOSE_WAIT, TIME_WAIT, FIN_WAIT: Разные стадии завершения соединения.
0.0.0.0 в поле Local Address означает, что служба привязана ко всем доступным сетевым интерфейсам устройства. Это значит, что она принимает входящие соединения с любых IP-адресов.
...
- По выводу команды top определи и напиши в отчёте:
  ...
  ![alt text](<screenshots/image copy 27.png>)
- В отчёт вставь скрин с выводом команды htop:
  - отсортированному по PID, PERCENT_CPU, PERCENT_MEM, TIME;
  pid ![alt text](<screenshots/image copy 28.png>)
  percent_cpu ![alt text](<screenshots/image copy 29.png>)
  percent_mem ![alt text](<screenshots/image copy 30.png>)
  time ![alt text](<screenshots/image copy 31.png>)
  - отфильтрованному для процесса sshd; ![alt text](<screenshots/image copy 32.png>)
  - с процессом syslog, найденным, используя поиск; ![alt text](<screenshots/image copy 33.png>)
  - с добавленным выводом hostname, clock и uptime. ![alt text](<screenshots/image copy 34.png>)
...
![alt text](<screenshots/image copy 35.png>)
- В отчёте напиши название жесткого диска, его размер и количество секторов, а также размер swap.
название ubuntu--vg-ubuntu-lv размер 11,51GiB секторов 24109056  
размер swap ![alt text](<screenshots/image copy 36.png>)
...
- В отчёте напиши для корневого раздела (/):
  ...
  ![alt text](<screenshots/image copy 37.png>)
...
![alt text](<screenshots/image copy 38.png>)
...
![alt text](<screenshots/image copy 39.png>) home
![alt text](<screenshots/image copy 40.png>) var
![alt text](<screenshots/image copy 41.png>) var/log
##### Выведи размер всего содержимого в /var/log (не общее, а каждого вложенного элемента, используя *).
![alt text](<screenshots/image copy 41.png>)
...
![alt text](<screenshots/image copy 42.png>)
![alt text](<screenshots/image copy 43.png>) /var/log
...
![alt text](<screenshots/image copy 44.png>)
- Перезапусти службу SSHd.
![alt text](<screenshots/image copy 45.png>)
- Вставь в отчёт скрин с сообщением о рестарте службы (искать в логах).
![alt text](<screenshots/image copy 46.png>)
...
*/2 * * * * uptime >> ~/uptime_log.txt
- Вставь в отчёт скрины со строчками о выполнении и списком текущих задач.
![alt text](<screenshots/image copy 47.png>)
##### Удали все задания из планировщика заданий.
- В отчёт вставь скрин со списком текущих заданий для CRON.
![alt text](<screenshots/image copy 48.png>)